...
vendor/bin/tester -p phpdbg tests -s -C --coverage ./coverage.xml --coverage-src ./src
```
...
# Check coding style with Nette Code Checker
composer create-project nette/code-checker temp/code-checker ^3 --no-progress
php temp/code-checker/code-checker --strict-types --no-progress --ignore expected

# Check coding standard with Nette CS
...
When running tests on Windows, the user's global CLAUDE.md indicates:
- Use `php.bat` instead of `php` command
- Avoid bash shell commands
- Do NOT use `cmd.exe /c` wrapper

Example: `php.bat vendor/bin/tester tests -C -s`
