...
```bash
# Run all tests (composer script)
composer tester
# or directly:
vendor/bin/tester tests -s

# Run specific test directory
vendor/bin/tester tests/Application -s
...
**Important:** Tests use Nette Tester with `.phpt` extension (154 test files total).
...
**Typed templates:**
- Presenters/controls can use dedicated template classes extending `Bridges\ApplicationLatte\Template`
- Declared via `@property-read {PresenterName}Template $template` on the presenter
- Activated in `.latte` files via the `{templateType}` declaration for full IDE support
- See the "Type-safe templates" section under Template Lookup for the full pattern
...
├── Bridges.Tracy/      Tracy integration tests
├── Routers/            Router tests
├── Responses/          Response type tests
├── UI/                 Presenter & component tests
├── types/              PHPStan type-inference tests (analysed by phpstan)
...
- Opening brace of a function/method goes on its own line:
  ```php
  public function example(string $param): void
  {
      // method body
  }
  ```
