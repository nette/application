...
function testPresenterGetPresenter(Component $component): void
{
	...
	assertType(Presenter::class . '|null', $presenter);

	$presenterOrNull = $component->getPresenterIfExists();
...
function testControlCreateTemplate(Control $control): void
{
	$template = $control->createTemplate();
	assertType(Template::class, $template);

	$template = $control->createTemplate(DefaultTemplate::class);
	assertType(DefaultTemplate::class, $template);
}


function testPresenterCreateTemplate(Presenter $presenter): void
{
	$template = $presenter->createTemplate();
	assertType(Template::class, $template);

	$template = $presenter->createTemplate(DefaultTemplate::class);
	assertType(DefaultTemplate::class, $template);
}
