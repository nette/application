...
Assert::match('<?php %a% $this->global->uiControl->getComponent("form"); %a%->render(); echo %a%$this->filters->filter%a%); ?>', $compiler->expandMacro('control', 'form', 'filter')->openingCode);
...
Assert::match('<?php %a% $this->global->uiControl->getComponent("form"); %a%->renderType([\'param\' => 123]); ?>', $compiler->expandMacro('control', 'form:type, param => 123', '')->openingCode);
Assert::match('<?php %a% $this->global->uiControl->getComponent("form"); %a%->render(); echo %a%$this->filters->striptags%a%); ?>', $compiler->expandMacro('control', 'form', 'striptags')->openingCode);
