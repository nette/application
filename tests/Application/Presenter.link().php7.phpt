...
class TestPresenter extends Application\UI\Presenter
{
	...
	protected function startup()
	{
		...
		Assert::same("#error: Invalid value for parameter 'x' in method TestPresenter::handlebuy(), expected int.", $this->link('buy!', [1], (object) [1]));
		...
		Assert::same("#error: Invalid value for parameter 'x' in method TestPresenter::handlebuy(), expected int.", $this->link('buy!', 'x'));
		Assert::same("#error: Invalid value for parameter 'bool' in method TestPresenter::handlebuy(), expected bool.", $this->link('buy!', 1, 2, 3));
		Assert::same("#error: Invalid value for parameter 'x' in method TestPresenter::handlebuy(), expected int.", $this->link('buy!', [[]]));
		Assert::same('/index.php?action=default&do=buy&presenter=Test', $this->link('buy!'));
		Assert::same("#error: Invalid value for parameter 'x' in method TestPresenter::handlebuy(), expected int.", $this->link('buy!', [new stdClass]));
	}
	 ...
	 * @view: default
	 */
	public function handleBuy(int $x = 1, int $y, bool $bool, string $str)
	{
	}
